...
class Waymo2KITTI(object):
    ...
    def __init__(self,
                 ...
                 prefix,
                 workers=64,
                 scene_num=100, # max 798 for waymo train set and 202 for val set
                 test_mode=False):
        ...
        self.save_velocity = True # save velocity label for scene flow estimation
        self.scene_num = scene_num # select waymo scene number
        ...
        self.label_save_dir_list = ['{}/{:03d}/label_'.format(self.save_dir, scene_id) for scene_id in range(self.scene_num)]
        self.label_all_save_dir_list = ['{}/{:03d}/label_all'.format(self.save_dir, scene_id) for scene_id in range(self.scene_num)]
        self.image_save_dir_list = ['{}/{:03d}/image_'.format(self.save_dir, scene_id) for scene_id in range(self.scene_num)]
        self.calib_save_dir_list = ['{}/{:03d}/calib'.format(self.save_dir, scene_id) for scene_id in range(self.scene_num)]
        self.point_cloud_save_dir_list = ['{}/{:03d}/velodyne'.format(self.save_dir, scene_id) for scene_id in range(self.scene_num)]
        self.pose_save_dir_list = ['{}/{:03d}/pose'.format(self.save_dir, scene_id) for scene_id in range(self.scene_num)]
    ...
    def save_image(self, frame, file_idx, frame_idx):
        ...
        for img in frame.images:
            img_path = f'{self.image_save_dir_list[file_idx]}{str(img.name - 1)}/' + \
    ...
    def save_calib(self, frame, file_idx, frame_idx):
        ...
        for camera in frame.context.camera_calibrations:
            ...
            camera_calib[0, 0] = camera.intrinsic[0]#fx
            camera_calib[1, 1] = camera.intrinsic[1]#fy
            camera_calib[0, 2] = camera.intrinsic[2]#cx
            camera_calib[1, 2] = camera.intrinsic[3]#cy
            camera_calib[2, 2] = 1
        ...
        with open(
                f'{self.calib_save_dir_list[file_idx]}/{self.prefix}' +
    ...
    def save_lidar(self, frame, file_idx, frame_idx):
        ...
        Args:
            ...
            frame_idx (int): Current frame index.
        """
        data = parse_range_image_and_camera_projection(frame)
        if len(data) == 4:
            range_images, camera_projections, segmentation_labels, range_image_top_pose = \
                data
        elif len(data) == 3:
            raise ValueError(
                'You must install waymo-open-dataset-tf-2-6-0 or a higher version, '+ \
                'and the waymo dataset must >= 1.3.0. '+ \
                'Please check GitHub repo for more details.\n'+ \
                'If you want to use existing crop parameters, ' + \
                'please remove this error and change the pc_num_features to 6 when creating scene flow data.')
            range_images, camera_projections, range_image_top_pose = data
            segmentation_labels = None
        
        # First return
        points_0, cp_points_0, intensity_0, elongation_0, point_labels_0 = \
        ...
        # Second return
        points_1, cp_points_1, intensity_1, elongation_1, point_labels_1 = \
            self.convert_range_image_to_point_cloud(
                frame,
                range_images,
                camera_projections,
        ...
        if segmentation_labels is not None:
            ...
            # concatenate x,y,z, intensity, elongation, timestamp, instance_id, semantic_id (8-dim)
            point_cloud = np.column_stack(
                (points, intensity, elongation, timestamp, point_labels))
        else:
            # concatenate x,y,z, intensity, elongation, timestamp (6-dim)
            point_cloud = np.column_stack(
                (points, intensity, elongation, timestamp))

        pc_path = f'{self.point_cloud_save_dir_list[file_idx]}/{self.prefix}' + \
    ...
    def save_label(self, frame, file_idx, frame_idx):
        ...
        """
        fp_label_all = open(
            f'{self.label_all_save_dir_list[file_idx]}/{self.prefix}' +
        ...
        for obj in frame.laser_labels:
            ...
            x = obj.box.center_x
            y = obj.box.center_y
            z = obj.box.center_z - height / 2 #change to bottom center
            ...
            if self.save_track_id and not self.save_velocity:
                line_all = line[:-1] + ' ' + name + ' ' + track_id + '\n'
            elif self.save_track_id and self.save_velocity:
                line_all = line[:-1] + ' ' + name + ' ' + track_id + ' ' + str(round(speed_x, 2)) + ' ' + str(round(speed_y, 2)) + '\n'
            else:
                line_all = line[:-1] + ' ' + name + '\n'

            fp_label = open(
                f'{self.label_save_dir_list[file_idx]}{name}/{self.prefix}' +
    ...
    def save_pose(self, frame, file_idx, frame_idx):
        ...
        np.savetxt(
            join(f'{self.pose_save_dir_list[file_idx]}/{self.prefix}' +
                 f'{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}.txt'),
            pose)
    ...
    def create_folder(self):
        ...
        for i in range(self.scene_num):
            if not self.test_mode:
                dir_list1 = [
                    self.label_all_save_dir_list[i], self.calib_save_dir_list[i],
                    self.point_cloud_save_dir_list[i], self.pose_save_dir_list[i]
                ]
                dir_list2 = [self.label_save_dir_list[i], self.image_save_dir_list[i]]
            else:
                dir_list1 = [
                    self.calib_save_dir_list[i], self.point_cloud_save_dir_list[i],
                    self.pose_save_dir_list[i]
                ]
                dir_list2 = [self.image_save_dir_list[i]]
            for d in dir_list1:
    ...
    def convert_range_image_to_point_cloud(self,
        ...
        return points, cp_points, intensity, elongation, point_labels
