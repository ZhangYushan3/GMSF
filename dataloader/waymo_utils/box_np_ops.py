...
def get_T_box_to_lidar(data):
    """Get 4*4 transform matrx for box to lidar coordinate.

    Args:
        data (np.ndarray, shape=[N, 7]): Boxes in lidar coordinate.

    Returns:
        np.ndarray, shape=[N, 4, 4]: Transform matrx.
    """
    num_boxes = data.shape[0]
    T_box2lidar = np.repeat(np.eye(4)[np.newaxis, ...], num_boxes, axis=0)
    rotz = data[:, 6]
    rot_cos, rot_sin = np.cos(rotz), np.sin(rotz)
    T_box2lidar[:,:2,:2] = np.stack(
        [rot_cos, -rot_sin, rot_sin, rot_cos], axis=-1).reshape(num_boxes,2,2)
    T_box2lidar[:,:3,3] = data[:, :3]
    return T_box2lidar
